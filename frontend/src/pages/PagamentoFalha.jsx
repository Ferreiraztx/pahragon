...
export default function PagamentoFalha() {
  ...
  function lidarTentarNovamente() {
    // Busca robusta pelo bookingId na URL usando Regex, ignorando bagunças do Mercado Pago
    const match = window.location.search.match(/[?&]bookingId=(\d+)/);
    const bookingId = match ? match[1] : null;

    if (bookingId) {
      // Redireciona direto para a tela de processamento do Pix/Mercado Pago
      navigate(`/pagamento/aguardando?bookingId=${bookingId}`);
    } else {
      // Caso não ache de jeito nenhum, volta pro fluxo seguro
      navigate("/agendar");
    }
  }
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center space-y-8 shadow-sm relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-sm leading-relaxed max-w-xs mx-auto">
              Não conseguimos processar o seu pagamento. Nenhuma cobrança foi realizada e seu horário continua reservado aguardando a conclusão.
            </p>
          </div>
